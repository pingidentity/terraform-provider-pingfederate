...
func (r *localIdentityIdentityProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	apiReadLocalIdentityIdentityProfile, httpResp, err := r.apiClient.LocalIdentityIdentityProfilesAPI.GetIdentityProfile(config.DetermineAuthContext(ctx, r.providerConfig), state.Id.ValueString()).Execute()
	if err != nil {
