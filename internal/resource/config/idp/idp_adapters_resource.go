...
func readIdpAdapterResponse(ctx context.Context, r *client.IdpAdapter, state *idpAdapterResourceModel, diags *diag.Diagnostics) {
	...
	state.PluginDescriptorRef = internaltypes.ToStateResourceLink(ctx, r.GetPluginDescriptorRef())
	state.ParentRef = internaltypes.ToStateResourceLink(ctx, r.GetParentRef())
