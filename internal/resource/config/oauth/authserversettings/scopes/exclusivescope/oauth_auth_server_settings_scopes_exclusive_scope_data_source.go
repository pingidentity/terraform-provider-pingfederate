...
func (r *oauthAuthServerSettingsScopesExclusiveScopeDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	apiReadOauthAuthServerSettingsScopesExclusiveScope, httpResp, err := r.apiClient.OauthAuthServerSettingsAPI.GetExclusiveScope(config.DetermineAuthContext(ctx, r.providerConfig), state.Name.ValueString()).Execute()
