...
func (r *oauthAuthServerSettingsScopesCommonScopeDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	apiReadOauthAuthServerSettingsScopesCommonScope, httpResp, err := r.apiClient.OauthAuthServerSettingsAPI.GetCommonScope(config.DetermineAuthContext(ctx, r.providerConfig), state.Name.ValueString()).Execute()
