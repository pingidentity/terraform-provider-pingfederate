...
func (r *oauthClientSettingsResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	if internaltypes.IsDefined(config.DynamicClientRegistration) {
		...
		if !attrs["require_offline_access_scope_to_issue_refresh_tokens"].IsUnknown() && !attrs["require_offline_access_scope_to_issue_refresh_tokens"].IsNull() && attrs["require_offline_access_scope_to_issue_refresh_tokens"].(types.String).ValueString() != "YES" &&
			!attrs["offline_access_require_consent_prompt"].IsUnknown() && !attrs["offline_access_require_consent_prompt"].IsNull() && attrs["offline_access_require_consent_prompt"].(types.String).ValueString() != "SERVER_DEFAULT" {
