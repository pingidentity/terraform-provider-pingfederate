...
func (r *keyPairsSslServerImportDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	apiReadKeyPairsSslServerImport, httpResp, err := r.apiClient.KeyPairsSslServerAPI.GetSslServerKeyPair(config.DetermineAuthContext(ctx, r.providerConfig), state.Id.ValueString()).Execute()
	if err != nil {
