...
func TestAccSpIdpConnection_InboundProvisioningPingFederate12_3(t *testing.T) {
	// Check if the server is at least PF 12.3 for this resource
	if !acctest.VersionAtLeast(version.PingFederate1230) {
		t.Skip("This server is not at least PingFederate 12.3, skipping TestAccSpIdpConnection_InboundProvisioningPingFederate12_3")
	}
	resource.Test(t, resource.TestCase{
		PreCheck: func() { acctest.ConfigurationPreCheck(t) },
		ProtoV6ProviderFactories: map[string]func() (tfprotov6.ProviderServer, error){
		...
		Steps: []resource.TestStep{
			{
				// Create the resource with PF 12.3 attrs
				Config: spIdpConnection_InboundProvisioningPingFederate12_3HCL(),
			},
			{
				// Test importing the resource
				Config:            spIdpConnection_InboundProvisioningPingFederate12_3HCL(),
			...
			{
				// Test switching to SCIM 1 from SCIM 2
				Config: spIdpConnection_InboundProvisioningCompleteHCL(),
				Check:  spIdpConnection_CheckComputedValuesInboundProvisioningComplete(),
			},
		},
...
// Maximal HCL with all values set where possible
func spIdpConnection_InboundProvisioningPingFederate12_3HCL() string {
	return fmt.Sprintf(`
%s
...
func spIdpConnection_CheckComputedValuesInboundProvisioningMinimal() resource.TestCheckFunc {
	...
	if acctest.VersionAtLeast(version.PingFederate1230) {
		testChecks = append(testChecks, resource.TestCheckResourceAttr("pingfederate_sp_idp_connection.example", "inbound_provisioning.scim_version", "SCIM11"))
	}
