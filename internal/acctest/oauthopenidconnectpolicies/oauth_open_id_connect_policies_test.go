...
const oauthOpenIdConnectPoliciesId = "2"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Config:                  testAccOauthOpenIdConnectPolicies(resourceName, updatedResourceModel),
				ResourceName:            "pingfederate_oauth_open_id_connect_policies." + resourceName,
...
func testAccOauthOpenIdConnectPolicies(resourceName string, resourceModel oauthOpenIdConnectPoliciesResourceModel) string {
	return fmt.Sprintf(`
resource "pingfederate_oauth_open_id_connect_policies" "%[1]s" {
	id = "%[2]s"
	FILL THIS IN
}`, resourceName,
		resourceModel.id,
...
func testAccCheckExpectedOauthOpenIdConnectPoliciesAttributes(config oauthOpenIdConnectPoliciesResourceModel) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		resourceType := "OauthOpenIdConnectPolicies"
		testClient := acctest.TestClient()
		ctx := acctest.TestBasicAuthContext()
		response, _, err := testClient.<RESOURCE_API>.GetOauthOpenIdConnectPolicies(ctx, oauthOpenIdConnectPoliciesId).Execute()
...
func testAccCheckOauthOpenIdConnectPoliciesDestroy(s *terraform.State) error {
	testClient := acctest.TestClient()
	ctx := acctest.TestBasicAuthContext()
	_, err := testClient.<RESOURCE_API>.DeleteOauthOpenIdConnectPolicies(ctx, oauthOpenIdConnectPoliciesId).Execute()
	if err == nil {
		return acctest.ExpectedDestroyError("OauthOpenIdConnectPolicies", oauthOpenIdConnectPoliciesId)
	}
	return nil
}
