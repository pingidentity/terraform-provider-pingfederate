...
- `client_cert_header_encoding_format` (String) Specify the encoding format of the client certificate header. The default value is `APACHE_MOD_SSL`. Supported values are `APACHE_MOD_SSL`, `NGINX`.
...
- `enable_client_cert_header_auth` (Boolean) Enable client certificate header authentication. Default value is `false`.
