...
- `verify_hostname` (Boolean) If `use_ssl` or `use_tls` is enabled, this flag determines whether the email server hostname is verified against the server's SMTPS certificate. Default is `false`.
