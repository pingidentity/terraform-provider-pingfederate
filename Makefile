...
testacc:
	$(call test_acc_common_env_vars) $(call test_acc_basic_auth_env_vars) TF_ACC=1 go test `go list ./internal/acctest/config... | grep -v -e authenticationapi -e oauth/authserversettings -e oauth/openidconnect/policy -e oauth/openidconnect/settings -e serversettings/systemkeys/rotate` -timeout 10m -v -p 4; \
	firstTestResult=$$?; \
	$(call test_acc_common_env_vars) $(call test_acc_basic_auth_env_vars) TF_ACC=1 go test `go list ./internal/acctest/config... | grep -e authenticationapi -e oauth/authserversettings -e oauth/openidconnect/policy -e oauth/openidconnect/settings -e serversettings/systemkeys/rotate` -timeout 10m -v -p 1; \
	secondTestResult=$$?; \
