...
testoneacc:
	$(call test_acc_env_vars) TF_ACC=1 go test ./internal/acctest/config/... -timeout 10m -run ${ACC_TEST_NAME} -v count=1
...
testacc:
	$(call test_acc_env_vars) TF_ACC=1 go test `go list ./internal/acctest/config/... | grep -v -e authenticationapi -e oauth/authserversettings/scopes -e oauth/openidconnect/policy` -timeout 10m -v -p 4; \
	firstTestResult=$$?; \
	$(call test_acc_env_vars) TF_ACC=1 go test `go list ./internal/acctest/config/... | grep -e authenticationapi -e oauth/authserversettings/scopes -e oauth/openidconnect/policy` -timeout 10m -v -p 1; \
	secondTestResult=$$?; \
