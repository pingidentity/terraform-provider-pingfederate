...
define productversiondir
 	PRODUCT_VERSION_DIR=$$(echo "$${PINGFEDERATE_PROVIDER_PRODUCT_VERSION:-12.1.4}" | cut -b 1-4)
endef
...
starttestcontainer:
	$(call productversiondir) && docker run --name pingfederate_terraform_provider_container \
		...
		pingidentity/pingfederate:$${PINGFEDERATE_PROVIDER_PRODUCT_VERSION:-12.1.4}-latest
