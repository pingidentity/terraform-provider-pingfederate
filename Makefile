...
define productversiondir
	export PRODUCT_VERSION_DIR=$$(echo $${PINGFEDERATE_PROVIDER_PRODUCT_VERSION:-12.0.1} | grep -E '\d\d\.\d' -o) && echo $${PRODUCT_VERSION_DIR} && test -d server-profiles/$${PRODUCT_VERSION_DIR} || { echo "No server-profiles/$${PRODUCT_VERSION_DIR} directory found. Exiting."; exit 1; }
endef
